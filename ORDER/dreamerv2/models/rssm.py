...
class RSSM(nn.Module, RSSMUtils):
    def __init__(
        ...
        self.use_prev_reward = use_prev_reward
        self.fc_embed_state_action = self._build_embed_state_action(use_prev_reward)
        self.fc_prior = self._build_temporal_prior()
        self.fc_posterior = self._build_temporal_posterior()
    
    def _build_embed_state_action(self, use_prev_reward=False):
        """
        model is supposed to take in previous stochastic state and previous action 
        and embed it to deter size for rnn input
        """
        if use_prev_reward:
            fc_embed_state_action = [nn.Linear(self.stoch_size + self.action_size + 1, self.deter_size)]
        else:
    ...
    def rssm_imagine(self, prev_action, prev_rssm_state, nonterms=True, prev_reward=None):
        if prev_reward is None:
            ...
            state_action_embed = self.fc_embed_state_action(
                torch.cat([prev_rssm_state.stoch * nonterms, prev_action, prev_reward], dim=-1))
    ...
    def rssm_observe(self, obs_embed, prev_action, prev_nonterm, prev_rssm_state, prev_reward=None):
        prior_rssm_state = self.rssm_imagine(prev_action, prev_rssm_state, prev_nonterm, prev_reward)
    ...
    def rollout_observation(self,  obs_embed: torch.Tensor, action: torch.Tensor, nonterms: torch.Tensor, prev_rssm_state,reward=None):
        priors = []
        posteriors = []
        ...
        for t in range(seq_len):
            ...
            if reward is not None:
                prev_reward = reward[t]*nonterms[t]
            else:
                prev_reward = None
            prior_rssm_state, posterior_rssm_state = self.rssm_observe(obs_embed[t], prev_action, nonterms[t], prev_rssm_state, prev_reward)
