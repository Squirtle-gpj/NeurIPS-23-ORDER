...
def main(config):
    ...
    env, dataset = get_env_and_dataset(config.env_name, config.max_episode_steps, sample_seq_length=None, codebook_size=config.n_code)
    ...
    def eval_policy():
        eval_returns = np.array([evaluate_policy_obs_encoder(env, obs_encoder, policy, config.max_episode_steps, label_input=config.label_input) \
    ...
    for step in trange(config.n_steps):
        ...
        results = iql.update_v2(batch['observations'], batch['actions'], config.label_input)
        #iql.update()
...
if __name__ == '__main__':
    ...
    parser.add_argument("--obs_encoder_path",
                        default='../Experiments/test/Checkpoints/train_state_encoder_1234/best.pt',
                        help="", type=str)
    parser.add_argument("--critic_path",
                        default='../Experiments/test/Checkpoints/train_critic_1234/best.pt',
                        help="", type=str)
    parser.add_argument('--deterministic_policy', action='store_true')
    parser.add_argument('--label_input', action='store_true')
    ...
    config = Config(args)
    config.load_config("vae_policy_v2", config.cfg_filename, format="yaml")
