...
class QuantileDiscretizer:
	...
	def __init__(self, data, N):
		...
		self.maxs = maxs.flatten()
		self.mins = mins.flatten()
	...
	def init_vae_weight(self):
		#weight = (self.thresholds[:self.N] + self.thresholds[self.N:] )/2
		ranges = [np.linspace(min_val, max_val, self.N) for min_val, max_val in zip(self.mins, self.maxs)]

		# Stack the arrays to get the desired shape (N, K)
		result = np.stack(ranges, axis=-1)
		return result
